...
const utilsApiMutationsExerciseSetCreate = async ({ args, tx, versionNext }: { args: { exercise: Exercise, exerciseSet: ExerciseSet }, tx: Omit<PrismaClient, "$connect" | "$disconnect" | "$on" | "$transaction" | "$use" | "$extends">, versionNext: number }) => {
    const prismaExerciseSetFindUnique = await tx.exerciseSet.findUnique({ where: { id: args.exerciseSet.id } })
