...
// Mutations
const mutationsExerciseDelete = async ({ tx, args }: { tx: WriteTransaction, args: ExerciseFull }) => {
    ...
    const exerciseIndex = session.exercises.findIndex((exercise) => exercise.id === args.id);
