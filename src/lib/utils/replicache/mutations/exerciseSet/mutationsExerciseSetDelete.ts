...
// Mutations
const mutationsExerciseSetDelete = async ({ tx, args }: { tx: WriteTransaction, args: { exercise: ExerciseFull, exerciseSet: ExerciseSet } }) => {
    ...
    if (!exercise.sets)
        throw new Error('Exercise Set does not exist');

    // Find index of set to delete
    const exerciseSetIndex = exercise.sets.findIndex(set => set.id === args.exerciseSet.id);
